...
def get_args_parser():
    ...
    parser.add_argument('--log_dir', default='linear_log/try1', type=str,
                        help='log dir')
    parser.add_argument('--dataset', default='chexpert', type=str,
                        help='dataset: chexpert, rsna')
    return parser
...
def main(args):
    ...
    # yaoyinuo 2022.7.13
    # CheXpert linear classification
    if args.dataset == 'chexpert':
        train_dataset = datasetTrain_frt
        val_dataset = datasetValid_frt
        test_dataset = datasetTest_frt
    ...
    if args.dataset == 'rsna':
        ...
        test_dataset = RSNA(csv_file='/data2/yinuo/rsna/test.csv',
                            id_col = 'patientId',
                            target_col = 'Target',
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train_stats = train(train_loader, model, criterion, optimizer, epoch, writer, args)
        # train_stats = 0.5
        ...
        ifval = True
        val_stats = validate(val_loader, model, criterion, writer, ifval, epoch, args)
        val_auc = val_stats

        ifval = False
        test_stats = validate(test_loader, model, criterion, writer, ifval, epoch, args)
        test_auc = test_stats
        ...
        #              'epoch': epoch}
        log_stats = {'train_loss': train_stats,
                     'val_auc': val_auc,
                     'test_auc': test_auc,
                     'epoch': epoch}
...
def train(train_loader, model, criterion, optimizer, epoch, writer, args):
    ...
    for images, target in tqdm(train_loader):
        ...
        n_iters += i
        writer.add_scalar('loss/train', loss, n_iters)
        i += 1
...
def validate(val_loader, model, criterion, writer, ifval, epoch, args):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    # top1 = AverageMeter('Acc@1', ':6.2f')
    # top5 = AverageMeter('Acc@5', ':6.2f')
    ...
    with torch.no_grad():
        ...
        aucs = []
        if args.dataset == 'rsna':
            pred = torch.max(F.softmax(outPRED), dim=1)[1]
        ...
        if args.dataset == 'chexpert':
            ...
            if ifval:
                writer.add_scalar('val/auc1', aucs[0], epoch)
                writer.add_scalar('val/auc2', aucs[1], epoch)
                writer.add_scalar('val/auc3', aucs[2], epoch)
                writer.add_scalar('val/auc4', aucs[3], epoch)
                writer.add_scalar('val/auc5', aucs[4], epoch)
                writer.add_scalar('val/meanAUC',auc_mean, epoch)
            else:
                writer.add_scalar('test/auc1', aucs[0], epoch)
                writer.add_scalar('test/auc2', aucs[1], epoch)
                writer.add_scalar('test/auc3', aucs[2], epoch)
                writer.add_scalar('test/auc4', aucs[3], epoch)
                writer.add_scalar('test/auc5', aucs[4], epoch)
                writer.add_scalar('test/meanAUC',auc_mean, epoch)
