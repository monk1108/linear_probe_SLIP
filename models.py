...
class CLIP(nn.Module):
    ...
    def encode_text(self, text):
        ...
        encoded_inputs = self.tokenizer(text, return_tensors="pt", padding=True, max_length=112, truncation=True)
        input_ids = encoded_inputs['input_ids'].cuda()
        mask = encoded_inputs['attention_mask'].cuda()
        del encoded_inputs          # otherwise will lead to memory increasing
        # encoded_inputs.cuda()
        outputs = self.bert_model(input_ids, mask)
        
        # yaoyinuo
        # code from ConVIRT
        # with torch.no_grad():
        sentence_embeddings = self.mean_pooling(outputs, mask).half()
        x = self.bert_l1(sentence_embeddings.to(torch.float32))
    ...
    def forward(self, image, text):
        image_embed = self.encode_image(image)    # 512
